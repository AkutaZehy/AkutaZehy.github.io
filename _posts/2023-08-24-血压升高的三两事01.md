...
但是怪就怪在 Wu-kan 的这个个人 repo，它没按正常的搞，这个 repo 实际上只有 `root` 和 `root/_posts` 两个结构，其中 post 的文章布局样式被放在了 `_config.yml` 的 `default/values/jekyll-theme-WuK/post` 这个级别下了，也就是 Front Matter 里面的配置。但是 tags 并不在这里。
